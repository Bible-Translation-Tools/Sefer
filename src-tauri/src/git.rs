...
use git2::{
    build::CheckoutBuilder, Cred, ErrorCode, FetchOptions, ObjectType, Oid, PushOptions,
    RemoteCallbacks, Repository, RepositoryInitOptions, Signature, Sort, Status, StatusOptions,
};
use serde::Serialize;
...
pub fn git_commit(
    ...
    let signature = Signature::now(&author_name, &author_email).map_err(io)?;
