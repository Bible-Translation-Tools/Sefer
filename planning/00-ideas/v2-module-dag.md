...
| Source and Book | 04 | sync core | canonical UTF-8 text per book, `SourceStamp {revision, len, hash}`, book open/close | `open(bytes) → Book`, `book.text`, `book.stamp`, `book.apply(change) → stamp`, `subscribe`, `close` |
