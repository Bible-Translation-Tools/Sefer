...
| `project.close`                                          | closing the open Project, which every open of another one does first                         | `project.books`; the `project.close` note inside it adds `project.seats`                                                                                                                                                                                        |
...
| `import.resource` / `import.remote`                      | importing a picked folder or ZIP / cloning a shared project, from the hub or a catalogue row | stage timings, counts, outcome and failing phase; `import.source` says which door; no URL or name. A clone ends through `remoteVerdict`                                                                                                                         |
| `catalogue.browse`                                       | the Find Project table's one read                                                            | `catalogue.source` (`live` or `sample`), `catalogue.entries`, `catalogue.gateways`; on failure `catalogue.failure` and `http.status`. A server down or a rejected fetch is `unavailable`, an unreadable payload `failed`                                        |
| `find.run`                                               | a valid search                                                                               | scope, `find.regex`/`find.case`/`find.whole_word`/`find.markup`, `find.chars` (the query's length), scan time, `find.hits`, `find.hit_books`, or the refusal; never the query text                                                                              |
| `findings.navigate`                                      | a jump from the Findings panel to the editor                                                 | `finding.code`, `finding.severity`, `finding.producer`, `findings.via` (`go`, `key`, `card`), `findings.resolved`, `findings.stale`; the `findings.openInEditor` span inside it; never the message                                                              |
| `reference.load`                                         | a reference pane reading its text                                                            | `reference.role`, `reference.outcome` (`text`, `missing`, `failed`), and for a text its block and verse counts and the open book's `reference.unpaired_blocks`, `reference.unpaired_empty` and `reference.unpaired_verses`; `reference.skeleton` span inside it |
| `review.compare`                                         | each comparison /review takes                                                                | `review.left`/`review.right` (picker kind), book, changed, one-sided and hunk counts, `review.units`; `review.books` and `review.units` spans inside it. `refused` when the engine has no diff door                                                             |
| `review.apply`                                           | applying a review plan                                                                       | requested/written counts, target side, refusal                                                                                                                                                                                                                  |
| `sync.survey`                                            | each reading /cloud takes                                                                    | `sync.state`, `sync.ahead`, `sync.behind`, `sync.remote`, `sync.signed_in`, `sync.online`, `sync.fetched_at`, `sync.uncommitted`, `sync.merge`; `unavailable` when the state is `offline`. Never the origin or the account                                      |
| `sync.plan`                                              | a survey that found the device behind (`op.cause` is the survey)                             | `sync.books`, `sync.contested`, `sync.chapters`, `sync.overlap`, `sync.clean`                                                                                                                                                                                   |
| `sync.transfer`                                          | the transfer action                                                                          | action, time, outcome; a failure ends through `remoteVerdict`                                                                                                                                                                                                   |
| `terms.load` / `terms.select`                            | /terms loading a guide / opening one term                                                    | `terms.locale`, `terms.count`, `terms.references` / the term's references, `terms.occurrences` found in the project, `terms.dropped`, `terms.books`, `terms.readings`; `terms.map` span inside it                                                               |
| `inventory.load`                                         | /inventory, from mount to its first paint                                                    | glyph, flagged, pattern and book counts; `declined` when left before it painted                                                                                                                                                                                 |
| `update.check` / `update.install`                        | the About panel's two buttons                                                                | `update.channel`, `update.result` / `update.reason`; an updater that cannot answer is `unavailable`, a refused install `failed`                                                                                                                                 |
| `journal.write`                                          | one debounced flush of the recovery journals (`op.cause` is the edit that armed it)          | `journal.books`, `journal.entries`, `journal.failed`; the per-journal `journal.write` notes inside it                                                                                                                                                           |
...
Inside those, as spans: `galley.parse` (with `galley.why` naming its caller), `file.write`, `corpus.publish`. As notes: `book.analyze`, `corpus.update`, `corpus.reference`, `baseline.adopt`, `seat.open`, `seat.close`, `project.metadata`, `book.apply` (only when no gesture is open), `journal.*`, `conflict.resolve`, `library.*`, and three that are notes by nature rather than operations: `project.watch` (a watch that gave up), `file.changed` (an external change the watcher saw) and `journal.pending` (the recovery scan's count). `reference.pair` (`refused`) is written once per empty block the reference pane's strict address match cannot pair.

`unavailable` is set at the outside boundaries only, in one place per boundary: `remoteVerdict` beside `RemoteFailureReason` in `src/core/remote/remote.ts` (`Unavailable`/`Network` → `unavailable`, `Unauthorized`/`Rejected` → `refused`, no reason → `failed`), `catalogueVerdict` in `src/app/catalogue.ts`, and `src/app/ui/UpdatePanel.tsx` for the Updater port.
