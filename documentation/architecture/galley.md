...
`Galley.diff(baseline, current)` and `Galley.merge(baseline, current, decisions, fallback)` exist, are typed, and **refuse**. `src/core/galley/diff.ts` is the whole of it: Onion's decision-unit wire as TypeScript, and one constant naming what is missing.

The diff itself is real. `onion/src/diff.rs` cuts each side into blocks at its own table-of-contents anchors (front matter, chapter open, verse), pairs them by a deliberately loose key — book + chapter + verse START, so a moved or rebridged verse still pairs and a renumbered one reads as a delete plus an add — and answers with a `DiffSkeleton`: an interleave of `Slot`s in which every byte of both inputs bears exactly one, plus a list of `DecisionUnit`s carrying `kind`, `status`, each side's own `Addr` and UTF-16 span, `displaced`, `relabeled`, a cross-document `DupContext`, a `CoveredBy` narration for a verse a bridge covers on the other side, and the two facts a review screen wants most — `isWhitespaceChange` and `isUsfmStructureChange`. `onion-wasm/src/lib.rs` binds `diff` (camelCase JSON, UTF-16 spans into each side's OWN document), `merge` and `mergeSplices`, and takes decisions back as `{"unitId": "baseline"|"current"}`. `onion::diff::unit_text_diff` computes word-grained intra-unit runs over the reader-visible bytes; it is not on the wasm wire yet, so `DecisionUnit.text` is optional here.

What is missing is the re-export on the artifact Sefer pins. `galley/src/wasm.rs` carries the `Galley` class and the init glue and no stateless Onion doors at all, so `vendor/galley/pkg-web/usfm_galley.d.ts` has no `diff`. `DIFF_DOOR` names it, exactly as `Fixes.FORMAT_DOOR` names the formatter's; both are asked for in `planning/01-discussing/engine-asks-2026-09-14.md`, items 1 and 1b.

Two shapes of the ask are worth recording, because they decide how this file is written:

- **The doors are FREE FUNCTIONS on the module, not methods on the handle.** Will, 2026-09-15: the next galley build exports `diff`, `merge`, `mergeSplices`, `format`, `formatEdits`, `formatEditsIn`, `toByte`, `toUtf16` and `locate`, plus the `FormatOpts`, `Edits` and `Splices` classes, at module level. They are stateless, so a handle method would be a claim about ownership that is not true; the handle keeps the corpus, the chunk cache and the knobs. So `engineDiff`/`engineMerge` probe the imported wasm MODULE namespace (`typeof module.diff === "function"`), and the day the artifact is regenerated the door opens with nothing else in Sefer to change.
- **The new wire reports BYTE offsets unless a `utf16` flag is passed.** Sefer speaks UTF-16 everywhere — `Analysis.docLen` is `text.length`, every `Change` and every `Book.apply` range is a JavaScript string index, and the find buffer is already decoded as UTF-16 — so this module passes `utf16: true` on every offset-bearing call and never converts. A byte offset that reached a `book.apply` would splice inside a character, which is the worst bug available here and one no type would catch.

Until then, `src/core/diff/skeleton.ts` builds the same `DiffSkeleton` from Sefer's own verse alignment, and `DiffSkeleton.engine` says which half produced the value in hand. What the interim can and cannot say is listed in [review.md](review.md).

## What the handle cannot do yet

The stateless Onion doors the spike used are **not** on this artifact: there is no `format`, `formatEdits`, `diff`, `merge`, `mergeSplices`, `attrs`, `attrResolve` or `locate`. A diagnostic still carries its own `fix()` edits, so the fix path works; whole-document formatting, the decision-unit diff (above), attribute resolution and the `locate` string do not. `find` and `findAll` were the previous entry on this list and are now on the handle (engine revision `f3a2b0b`); the pattern held — when Sefer needs one of these, the ask goes upstream to the engine's wasm surface and the adapter does not reimplement it.
