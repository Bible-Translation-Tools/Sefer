...
`src/editor/recipes/reference.ts` is the mount. `mountReference({ parent, text, analyze, mode })` answers a `ReferenceMount` with `setMode`, `clipTo`, `showChapter` and `destroy`. What it installs is the editor's READING half and nothing else:
