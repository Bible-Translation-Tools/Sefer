...
A page that needs a service the root does not provide merges a child Layer over the root's *built* services rather than recomposing: `composition.layer` is `Layer.succeedContext` of the context the root composition ran in, so `Effect.provide(program, Layer.merge(composition.layer, ChildLive))` reuses the same Observability ring and adds only what is new. The dev fixture page does exactly this with `FixtureFileSystemLive`, and its `reset` builds a fresh instance of that memory Layer without touching the composition.
