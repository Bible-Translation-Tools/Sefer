...
`src/platform/web/git.browser.test.ts` is the [plan 24](../../planning/00-ideas/v2-24-effect-filesystem-and-git-probe.md) Web probe: real isomorphic-git over `nodeFsView(OPFS)` performs `init`, `add`, `commit`, `log`, and `readBlob` against bytes written through the Effect `FileSystem`. It passes; the desktop half of that probe is still open.
