...
- **Memory** — `src/core/fileSystem/memory.ts` exports `MemoryFileSystemLive(seed?)` and `makeMemoryFileSystem(seed?)`. Present. Paths are normalised POSIX-style; `seed(entries)` populates it, which is how the dev fixture project is loaded. `open`, `link`, `symlink`, `readLink`, `chown`, `glob`, and `watch` are unimplemented and fail with a `PlatformError` naming the method; `stream` and `sink` derive from `open` and therefore fail too.
- **Tauri fs plugin** — `src/platform/tauri/fileSystem.ts` exports `TauriFileSystemLive`, composed on desktop. `watch` is real. `chmod`, `chown`, `copy`, `glob`, `link`, `open`, `readLink`, `symlink`, `truncate` and `utimes` are unimplemented and fail with a `PlatformError` naming the method.
- **Web OPFS** — `src/platform/web/fileSystem.ts` exports `OpfsFileSystemLive` over the Origin Private File System (`navigator.storage.getDirectory()`), composed on the Web. Paths are POSIX-style under the OPFS root and are normalised by the same `path.ts` helpers the memory layer uses. `chmod`, `chown`, `glob`, `link`, `makeTempFile`, `makeTempFileScoped`, `open`, `readLink`, `symlink`, `truncate`, `utimes`, and `watch` are unimplemented and fail with a `PlatformError` naming the method; `stream` and `sink` derive from `open` and therefore fail too.

## Helpers

`writeFileAtomic(fileSystem, path, bytes)` in `src/core/fileSystem/atomic.ts` writes to the deterministic sibling `<path>.sefer-tmp` and then renames it over the target, so a failed write leaves the previous bytes intact. The sibling name is deterministic so a crash leaves a predictable orphan that recovery can find; Sefer serialises writes per path. On failure the sibling is removed. `rename` is an atomic replace on the memory, Node and OPFS implementations; see the OPFS note below for how the Web layer achieves it.
...
When `move` is absent the fallback for a file is copy-then-remove: the source bytes are written to the target through `createWritable()`, whose `close()` swaps the completed file in, and the source entry is then removed. A reader therefore sees either the previous target bytes or the complete new bytes — never a torn file — but the pair is not atomic: a crash between `close()` and the source removal leaves both the target and an orphan source. That is the exposure `writeFileAtomic`'s deterministic `.sefer-tmp` sibling is named for, so recovery can find it. With `move` absent, renaming a _directory_ is refused with a `PlatformError` naming the method; there is no non-atomic directory-move fallback.
...
isomorphic-git 1.38.4 reads a global `Buffer` (`Buffer.from`, `Buffer.alloc`, `Buffer.concat`, `Buffer.isBuffer`) and no bundler supplies one to a browser build. The Web host installs one from the `buffer` package in `src/platform/web/git.ts` before calling into Git, which is the reason that package is a dependency. `nodeFsView` is production code on the Web: `src/platform/web/git.ts` and `src/platform/web/remote.ts` both run isomorphic-git over it.
