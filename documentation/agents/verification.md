...
Use deterministic Playwright tests for selected repeatable Web journeys. An exploratory session may reveal such a test, but its command transcript is not automatically a regression test. Avoid turning every exploratory action into permanent suite maintenance.

### Which browser to drive

Three routes exist and they are not interchangeable. Pick by what the run needs to SURVIVE, not by what is quickest to type.

| You need | Use | Why |
| --- | --- | --- |
| A page rendered, a screenshot, a DOM assertion — nothing persisted | `chromium.launch()`, ephemeral, in a throwaway script | No profile, no port, nothing to clean up, no chance of colliding with a rig someone else left running. Most verification is this. |
| A real project in OPFS, or anything measured across runs | `pnpm verify:chrome` + `chromium.connectOverCDP(...)` | The corpus is the reason: `en_ulb` is 66 books, imported once into `~/.sefer-cdp-profile` and still there next week. An ephemeral browser starts with an empty OPFS every time. |
| Proof that a JOURNEY keeps working | `pnpm test:e2e` (`e2e/`, against the built artifact) | It runs in CI. The other two only ever prove something on the machine that ran them. |

The rig is the expensive one — a long-lived profile, a fixed port, a warm cache that has faked three no-op results, and a `--stop` somebody has to remember. Reach for it when you need the corpus, not by default.

**`agent-browser` is installed** (`agent-browser --version`, 0.27.0, via mise) and is a reasonable lightweight front door. But every worked recipe below is Playwright over CDP, because that is what was actually run here and what the rig prints an endpoint for. Do not add a second harness to a task solely to satisfy a tool preference; one working driving route per task.
