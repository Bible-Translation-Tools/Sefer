...
| Census | agreed | Spent twice already, so do not spend it a third time: the glyph inventory behind `/inventory`, and the book-list page a Project opens onto. It is NOT the TOC — the engine's own name for the TOC buffer is "census", and Sefer renames it at the seam (`ProjectToc`, `BookToc`) for exactly this reason. |
...
| Corpus | agreed | Galley's registry of many Books' Source at once, so it can answer questions only true across Books. Reached through a port (`CorpusEngineService`), and since v0.1.1 there is ONE implementation: the wasm handle in this process, on both hosts. A second engine over IPC was tried and removed — the id doors answer off the text a handle retains, so a Corpus in another process is one the parse path cannot name. Not the Project, which is files on disk. |
