// diff.ts
//
// The DIFF DOOR: the engine's decision-unit diff, as Sefer's types.
//
// The engine is the only diff `/review` has. `diff`, `merge` and
// `mergeSplices` arrive as free functions on the galley wasm module, and
// `DIFF_DOOR` is the refusal an artifact without them produces, because the
// alternative is a screen that silently shows a second opinion about
// scripture structure.
//
...
//
// The engine's stateless doors arrive as FREE FUNCTIONS on the wasm module,
// under onion-wasm's own names — `diff`, `merge`, `mergeSplices`, `format`,
...
//
// A run arrives as `{from, to, kind, what}` — UTF-16 offsets into that side's
// own document, and no text. The runs TILE the unit's span, markup included,
// so `what` is what separates the reading from the markup: dropping the
// `"markup"` runs and concatenating the rest is the engine's `"text"` mask cut
// of the same span, which is the reading `review/reading.ts` asks the engine
// for. `decodeSkeleton` fills each run's `text` from the side it came from, so
// a consumer draws a run without holding the whole document.
//
...
/**
 * What a run's characters ARE. `markup` is marker, designator and attribute
 * bytes; the other two are what a reader sees, note prose included.
 */
export type RunWhat = "markup" | "text" | "whitespace";

export interface TextRun {
  /** UTF-16, into this side's own document, like the unit's own span. */
...
/**
 * Word or grapheme runs inside one unit, TILING each side's span
 * (`onion::diff::unit_text_diff`): every character of the span is in exactly
 * one run, markup included. `readingRuns` is the reader-visible subset.
 *
 ...
 * a screen that states which alignment it is showing is a screen a reviewer
 * can check.
 */
 ...
 * is grapheme clusters; `"none"` computes nothing and carries no runs at all.
 ...
 *
 * The doors are probed by name off the wasm module, so an artifact that is not
 * the pinned build is a real failure mode, and it should say so.
 */
 ...
 * is pinned in `package.json`. Seeing this string means the artifact in hand
...
const readRuns = (value: unknown, source: string): readonly TextRun[] => {
  if (!Array.isArray(value)) return [];
  const out: TextRun[] = [];
  for (const entry of value) {
    if (!isRecord(entry)) continue;
    const from = num(entry.from);
    const to = num(entry.to);
    if (to <= from) continue;
    out.push({
      from,
      to,
...
const readText = (
  ...
  baselineText: string,
  currentText: string,
): UnitTextDiff | undefined => {
  if (!isRecord(value)) return undefined;
  const baseline = readRuns(value.baseline, baselineText);
  const current = readRuns(value.current, currentText);
 ...
 * changed row and an unchanged one read the same way, notes included. The two
 * sides' `unchanged` runs stay paired in order: an unchanged run is the same
 * characters on both sides, so it is dropped from both or from neither.
 */
export const readingRuns = (runs: readonly TextRun[]): readonly TextRun[] =>
  runs.filter((run) => run.what !== "markup");
 ...
 *
 * Takes the two documents the JSON was computed from, because a run is a span
 * and its text is sliced out of its own side here.
 */
...
/**
 * The free functions the galley wasm module carries. Structural and optional,
