...
- `pnpm test:unit` runs the Node `core` Vitest project (core, app, and tools tests). There is no jsdom project.
...
Core modules stay independent of Solid, the router, Tauri, CodeMirror, DOM globals, and native filesystem implementations; `pnpm boundaries` is the authoritative check. TanStack Router owns navigation; it is not automatically the DI container. Effect is used today only for the boot program's typed failures; a scoped runtime and Layers arrive with the first real resource. Isomorphic Git is present as a dependency for later integration work, but its lifecycle and filesystem adapter are not established by the scaffold.
